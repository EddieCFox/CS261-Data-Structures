...
// Adding a prototype for a indexSmallest function here. The function compares the values of the left and right
// children, and returns the value of the smallest child. 

int _indexSmallest(struct DynamicArray*, int i, int j, compareFunction compare);


// Definition for _indexSmallest function here. If the value at index I is less or equal to
// the value at index J, based on the compare function, return index I as it is smaller than 
// index J. Otherwise, return index J.

int _indexSmallest(struct DynamicArray* heap, int i, int j, compareFunction compare)
{
...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
	...
	{
		if (_indexSmallest(heap, leftChild, rightChild, compare) == leftChild)
		{
			if (_indexSmallest(heap, position, leftChild, compare) == leftChild)
			{
				dySwap(heap, position, leftChild);
				adjustHeap(heap, last, leftChild, compare);
			}

			if (_indexSmallest(heap, position, rightChild, compare) == rightChild)
			{
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
	...
	{
		parent = (position - 1) / 2;
		TYPE parentValue = dyGet(heap, parent);
		TYPE positionValue = dyGet(heap, position);

		if (compare(positionValue, parentValue) < 0)
		{
