...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	if (compare(val, cur->val) == 0) /* If the value of the node matches the value of the parameter*/
	{
		if (cur->left == 0 && cur->right == 0) /* If there is no children, remove the node and return NULL.*/
		{
			free(cur);
			return 0;
		}

		else if (cur->left == 0) /* If there is no left child, return the right child and remove the node.*/
		{
		...
		else if (cur->right == 0) /* If there is no right child, return the left child and remove the node.*/
		{
		...
		else /* If there is both left and right children,*/
		{
			cur->val = _leftMost(cur->right); /* Replace current node with leftmost child of the right subtree.*/
			cur->right = _removeLeftMost(cur->right); /* Remove the actual leftmost child to prevent duplicates. Poor child.*/
			return cur;
		}
	}

	else if (compare(val, cur->val) < 0) /* If the value is less than the value of the node*/
	{
		cur->left = _removeNode(cur->left, val); /* Recursively traverse left through the tree*/
	}

	else if (compare(val, cur->val) > 0)
	{
		cur->right = _removeNode(cur->right, val); /* Recurisvely traverse right through the tree*/
	}

	return cur; /* Return current, used by recursive traversal calls to rebuild the tree*/
