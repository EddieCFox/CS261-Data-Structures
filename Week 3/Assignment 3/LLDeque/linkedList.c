...
static void addLinkBefore(struct LinkedList* list, struct Link* link, TYPE value)
{
	...
	newLink->value = value;
	newLink->next = link; //newLink placed before lnk provided as argument
...
{
	// FIXME: you must write this
	assert(!linkedListIsEmpty(list)); //make sure we have something to work with
	assert(link != list->frontSentinel && link != list->backSentinel);
	struct Link *temp = link; //for mem mgmt. later

	link->prev->next = link->next; //handle prev link
	link->next->prev = link->prev; //handle next link
